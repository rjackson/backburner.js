...
DeferredActionQueues.prototype.flush = function flush () {
    ...
    while (this.queueNameIndex < numberOfQueues) {
        ...
        if (queue.hasWork() === false) {
            ...
            this$1.queueNameIndex = 0; // only reset to first queue if non-pause break
        }
    }
};
...
var Backburner = function Backburner(queueNames, options) {
      ...
      this._platform = this.options._platform || {
          ...
          clearNext: function clearNext(fn) {
              clearTimeout(fn);
          }
      };
  ...
  */
  Backburner.prototype.scheduleIterable = function scheduleIterable (queueName, iterable) {
      var stack = this.DEBUG ? new Error() : undefined;
      var _iteratorDrain = iteratorDrain;
  ...
  Backburner.prototype._scheduleExpiredTimers = function _scheduleExpiredTimers () {
      ...
      for (; i < l; i += 2) {
          ...
          var fn = timers[i + 1];
          if (executeAt <= n) {
              this$1.schedule(this$1.options.defaultQueue, null, fn);
          }
          else {
              break;
          }
  ...
  Backburner.prototype._ensureInstance = function _ensureInstance () {
      var currentInstance = this.currentInstance;
      if (!currentInstance) {
          var next = this._platform.next || this._platform.setTimeout; // TODO: remove the fallback
